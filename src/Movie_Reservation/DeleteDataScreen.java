...
public class DeleteDataScreen extends JPanel {
	...
	DeleteDataScreen(ManagerScreen managerScreen) {
        ...
        submit.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				...
				try {
					...
					if (query.trim().toLowerCase().startsWith("delete") || query.trim().toLowerCase().startsWith("drop")) {
		                stmt.executeUpdate(query);
		            } else {
		                JOptionPane.showMessageDialog(null, "쿼리는 delete 또는 drop으로 시작해야 합니다.", "경고", JOptionPane.WARNING_MESSAGE);
		            }
        ...
        label = new JLabel("테이블 자체 삭제 : drop table 테이블명");
        font = label.getFont();
        ...
        label = new JLabel("테이블 값 삭제 : delete from 테이블명 where 조건");
        font = label.getFont();
        fontSize = font.getSize() + 10;
        label.setFont(font.deriveFont(fontSize));
        conditionPanel.add(label);
